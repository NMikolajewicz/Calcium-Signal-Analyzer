...
tic;
try
[fu2,spike2,imu2,fufine2,noise2] = TVRegSpikeRemoveMultiL1BiCont2(f, 3, [], 'large', 1e-3, 1 / length(f),1, 0,0);
...
% respflat=respdrift;
    deviation=fufine(ind,1)-respflat(ind);
  deviation(find(deviation(1:i50down-i50up+1)<0))=0;
  deviation(1)=0;
  deviation(end)=0;

  if isempty(dev_thresh)
    dev_thresh=6;
  end

    [crest,tcrest,wcrest,pcrest]=findpeaks(deviation,tcurr,'MinPeakProminence',dev_thresh*sigma,'MinPeakDistance',Tmin,'WidthReference','HalfProm');
    [trough,ttrough,wtrough,ptrough]=findpeaks(-deviation,tcurr,'MinPeakProminence',dev_thresh*sigma,'MinPeakDistance',Tmin,'WidthReference','HalfProm');
